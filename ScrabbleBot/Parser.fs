...
let palphanumeric =
    satisfy (fun x -> System.Char.IsLetterOrDigit x) <?> "alphanumeric"
...
let charListToString (lst: char list) =
    System.String.Concat(Array.ofList (lst))

let pid =
    pletter <|> pchar '_' .>>. many (palphanumeric <|> pchar '_')
    |>> fun (x, y) -> charListToString (x :: y)
...
let NegationParse =
    unop (pchar '-') TermParse |>> (fun x -> Mul((N -1), x)) <?> "Negation"
...
let CharParse =
    pchar ''' >>. (palphanumeric <|> whitespaceChar) .>> pchar ''' |>> C <?> "Char"
...
do
    cref
    := choice [ IntToCharParse; CVParse; ToUpperParse; ToLowerParse; CharParse ]

let CharToIntParse =
    unop pCharToInt (parenthesise CParse) |>> CharToInt <?> "CharToInt"

do
    aref
    := choice [ NegationParse; CharToIntParse; NParse; PVParse; VarParse; ParParse ]
